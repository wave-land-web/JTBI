...
export default defineType({
  ...
  fields: [
    defineField({
      name: 'title',
      title: 'Title',
      description: 'Name of the snapshot (used in cards and navigation).',
      type: 'string',
      group: 'content',
      validation: (Rule) => Rule.required().error('A snapshot title is required.'),
    }),
    defineField({
      name: 'description',
      title: 'Description',
      description: 'Short summary shown on the snapshot card.',
      type: 'text',
      rows: 2,
      group: 'content',
    ...
    defineField({
      name: 'image',
      title: 'Image',
      description: 'Image used on the snapshot card. Recommended: 800x800 (1:1 square).',
      type: 'image',
      group: 'content',
    ...
    defineField({
      name: 'href',
      title: 'Link',
      description: 'Optional link destination for the snapshot card. Supports relative paths.',
      type: 'string',
      group: 'link',
    }),
    defineField({
    ...
    defineField({
      ...
      fields: [
        ...
        defineField({
          name: 'titleColor',
          title: 'Title Color',
          description: 'Color of the snapshot title.',
          type: 'string',
          validation: cssColorValidation,
        }),
      ],
